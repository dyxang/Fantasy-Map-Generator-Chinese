  ...
  getBase(base: number, min?: number, max?: number, dupl?: string): string {
    ...
    if (nameBases[base] === undefined) {
      if (nameBases[0]) {
        ...
        ERROR && console.error(`名称库 ${base} 未找到`);
        return "ERROR";
      }
    }
    ...
    if (!data || data[""] === undefined) {
      tip(`名称库 ${base} 不正确。请在名称库编辑器中检查`, false, "error");
    ...
    if (name.length < 2) {
      ERROR && console.error("名称过短！将选择随机名称");
