...
document.addEventListener("DOMContentLoaded", async () => {
  if (!location.hostname) {
    ...
    alertMessage.innerHTML = /* html */ `幻想生成器不能无服务器运行. 跟随<a href="${wiki}" target="_blank">教程</a>学习如何简单运行一个本地服务器`;
...
async function checkLoadParameters() {
  ...
  if (params.get("maplink")) {
    ...
    if (valid) {
      ...
      return;
    } else showUploadErrorMessage("地图链接无效", maplink);
  }

  // if there is a seed (user of MFCG provided), generate map for it
  if (params.get("seed")) {
    WARN && console.warn("根据种子生成地图");
...
function findBurgForMFCG(params) {
  ...
  invokeActiveZooming();
  tip("这里矗立着辉煌的" + b.name, true, "成功之城", 15000);
}
...
void (function addDragToUpload() {
  ...
  document.addEventListener("drop", function (e) {
    ...
    overlay.innerHTML = "上传中<span>.</span><span>.</span><span>.</span>";
...
async function generate(options) {
  try {
    ...
    showStatistics();
    INFO && console.groupEnd("生成的地图 " + seed);
  } catch (error) {
    ERROR && console.error(error);
    const parsedError = parseError(error);
    clearMainTip();

    alertMessage.innerHTML = /* html */ `生成地图时出现错误。请重试。<br />如果错误严重，请清除存储的数据后重试。
...
function showStatistics() {
  ...
  const stats = `  种子: ${seed}
    ...
    国家: ${pack.states.length - 1}
    省份: ${pack.provinces.length - 1}
    城市: ${pack.burgs.length - 1}
    宗教: ${pack.religions.length - 1}
    文化设定: ${culturesSet.selectedOptions[0].innerText}
    文化: ${pack.cultures.length - 1}`;
