...
openCreateHeightmapSchemeButton.addEventListener("click", function () {
  ...
  function renderStops() {
    ...
    const colorInput = color =>
      `<input type="color" class="stop" value="${color}" data-tip="点击添加颜色" style="width: 2.5em; border: none;" />`;
    const removeStopButton = index =>
      `<button class="remove" data-index="${index}" data-tip="移除颜色边界" style="margin-top: 0.3em; height: max-content;">x</button>`;
    const addStopButton = () =>
      `<button class="add" data-tip="中间添加颜色边界" style="margin-top: 0.3em; height: max-content;">+</button>`;
  ...
  function renderGradient() {
    ...
    byId("heightmapSchemeGradient").style.background = `线性梯度（向右, ${stops})`;
  }
  ...
  $("#alert").dialog({
    ...
    buttons: {
      应用: function () {
        ...
        if (!name || name === "") return tip("请提供一个有效的 URL", false, "error");
