...
function editRoute(id) {
  ...
  tip(
    "拖动控制点以更改路线。点击点来移除它。点击路线添加额外的控制点。对于重大更改，请创建一个新路线代替",
    true
  );
  ...
  function openJoinRoutesDialog() {
    ...
    if (candidateRoutes.length) {
      ...
      alertMessage.innerHTML = /* html */ `<div>路线与之连接:
      ...
      $("#alert").dialog({
        ...
        buttons: {
          ...
          Join: () => {
            ...
            tip("路线连线了", false, "success", 5000);
            $("#alert").dialog("close");
          }
        }
      });
    } else {
      tip("没有可以连接的路线。路线必须在当前路线的起始或结束单元开始或结束", false, "error", 4000);
    }
  }
