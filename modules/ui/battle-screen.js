...
class Battle {
  ...
  addHeaders() {
    ...
    headers += "<th data-tip='军队总计'>总计</th></tr></thead>";
  ...
  addRegiment(side, regiment) {
    ...
    for (const u of options.military) {
      initial += `<td data-tip="原生力量" style="width: 2.5em; text-align: center">${
        regiment.u[u.name] || 0
      }</td>`;
      casualties += `<td data-tip="伤亡人员" style="width: 2.5em; text-align: center; color: red">0</td>`;
      survivors += `<td data-tip="幸存者" style="width: 2.5em; text-align: center; color: green">${
        regiment.u[u.name] || 0
      }</td>`;
    }

    initial += `<td data-tip="原生力量" style="width: 2.5em; text-align: center">${regiment.a || 0}</td></tr>`;
    casualties += `<td data-tip="伤亡人员"  style="width: 2.5em; text-align: center; color: red">0</td></tr>`;
    survivors += `<td data-tip="幸存者" style="width: 2.5em; text-align: center; color: green">${
      regiment.a || 0
    }</td></tr>`;
  ...
  applyResults() {
    ...
    const legend = `${this.name} 发生在 ${options.year} ${options.eraShort}. 这场战争发生于 ${getSide(
      this.attackers.regiments,
      1
    )} 和 ${getSide(this.defenders.regiments, 0)}. ${result}.
      \r\n进攻方损失: ${getLosses(this.attackers.casualties)}%, 防守方损失: ${getLosses(
