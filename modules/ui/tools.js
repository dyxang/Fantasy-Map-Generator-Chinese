...
function recreateStates() {
  ...
  if (!statesCount) {
    tip(`<i>国家数量</i>选项值为零. 无国家生成`, false, "error");
    return null;
  }

  const validBurgs = pack.burgs.filter(b => b.i && !b.removed);
  if (!validBurgs.length) {
    tip("没有任何城市用来产生国家。请先创建城市", false, "error");
    return null;
  }
  ...
  if (validStates.length && lockedStates.length === validStates.length) {
    tip("由于所有国家被锁定，因此无法重新生成", false, "error");
    return null;
  }
...
function addRiverOnClick() {
  ...
  while (i) {
    ...
    if (h[i] <= h[min]) return tip(`单元格 ${i} 凹陷, 河水不能再流了`, false, "error");
