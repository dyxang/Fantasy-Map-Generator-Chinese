...
function editZones() {
  ...
  function zonesEditorAddLines() {
    ...
    const lines = filteredZones.map(({i, name, type, cells, color, hidden}) => {
      ...
      return /* html */ `<div class="states" data-id="${i}" data-color="${color}" data-description="${name}"
        ...
        <input data-tip="区域描述。点击并键入以更改" style="width: 11em" class="zoneName" value="${name}" autocorrect="off" spellcheck="false">
        <input data-tip="区域类型。点击并键入以更改" class="zoneType" value="${type}">
        <span data-tip="单元格计数" class="icon-check-empty hide"></span>
        <div data-tip="单元格计数" class="stateCells hide">${cells.length}</div>
        ...
        <span data-tip="切换区域可见性" class="zoneHide icon-eye hide ${
          cells.length ? "" : " placeholder"
        }"></span>
        <span data-tip="删除区域" class="zoneRemove icon-trash-empty hide"></span>
      </div>`;
    });
  ...
  function changePopulation(zone) {
    ...
    if (!landCells.length) return tip("区内没有任何土壤单元格，不能改变人口", false, "error");
