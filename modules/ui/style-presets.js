...
async function getStylePreset(desiredPreset) {
  ...
  if (isCustom) {
    ...
    if (!storedStyleJSON) {
      ERROR && console.error(`在本地存储中找不到自定义样式${desiredPreset}。应用默认样式`);
      ...
      ERROR &&
        console.error(`在本地存储中的自定义样式${desiredPreset}无效。应用默认样式`);
      presetToLoad = "default";
    }
  }
...
function requestStylePresetChange(preset) {
  ...
  confirmationDialog({
    title: "更改样式预设",
    message: "您确定要更改样式预设吗?所有未保存的样式更改将丢失",
    confirm: "更改",
    onConfirm: () => {
...
function addStylePreset() {
  $("#styleSaver").dialog({title: "样式保存器", width: "26em", position: {my: "center", at: "center", of: "svg"}});
  ...
  function saveStyle() {
    ...
    if (!styleJSON) return tip("请提供 JSON 格式的样式文件", false, "error");
    if (!JSON.isValid(styleJSON)) return tip("JSON字符串无效，请检查格式", false, "error");
    if (!desiredName) return tip("请提供预设名称", false, "error");
    if (styleSaverTip.innerHTML === "default")
      return tip("您不能覆盖默认预设，请更改名称", false, "error");
    ...
    tip("样式预设已保存并应用", false, "success", 4000);
  ...
  function loadStyleFile() {
    ...
    function styleUpload(dataLoaded) {
      if (!dataLoaded) return tip("载入文件失败，检查数据格式", false, "error");
      const isValid = JSON.isValid(dataLoaded);
      if (!isValid) return tip("请检查格式，提供的不是一个有效JSON文件", false, "error");

      styleSaverJSON.value = JSON.stringify(JSON.parse(dataLoaded), null, 2);
      styleSaverName.value = fileName;
      checkName();
      tip("格式预设已上传", false, "success", 4000);
    }
  }
}
...
function requestRemoveStylePreset() {
  ...
  if (isDefault) return tip("不能删除默认预设", false, "error");

  confirmationDialog({
    title: "移除样式预设",
    message: "是否移除？此操作不可撤销！",
    confirm: "移除",
